...
namespace L1
{
    public partial class Form1 : Form
    {
        ...
        void FillData()
        {
            try
            {
                con = new SqlConnection("Data Source=DESKTOP-SBCUNGL\\SQLEXPRESS;Initial Catalog=ParkingLots;Integrated Security=true;");

                queryParent = "SELECT * FROM Zones";
                queryChild = "SELECT * FROM Lots";

                daParent = new SqlDataAdapter(queryParent, con);
                daChild = new SqlDataAdapter(queryChild, con);

                dset = new DataSet();
                daParent.Fill(dset, "Zones");
                daChild.Fill(dset, "Lots");

                cmdBuilder = new SqlCommandBuilder(daChild);

                dset.Relations.Add("ZonesLots", dset.Tables["Zones"].Columns["z_id"], dset.Tables["Lots"].Columns["z_id"]);

                bsParent = new BindingSource();
                bsParent.DataSource = dset.Tables["Zones"];
                bsChild = new BindingSource(bsParent, "ZonesLots");
                dgvZones.DataSource = bsParent;
                labelParent.Text = "Zones";
                dgvParkingLots.DataSource = bsChild;
                labelChild.Text = "Lots";
