...
{
	vector<Tutorial> tutorials = this->service.getTutorialsService();
	for (auto it = tutorials.begin(); it != tutorials.end(); it++)
		cout << (*it);
}

vector<Tutorial> Console::tutorialsGivenPresenterConsole()
{
	std::string given_string;
	std::cout << "\nPlease choose a presenter: ";
	std::getline(std::cin, given_string);
	vector<Tutorial> tutorials = this->service.tutorialsGivenPresenter(given_string);
	return tutorials;
}
...
void Console::iterateTutorials()
{
	...
	vector<Tutorial> tutorials = this->tutorialsGivenPresenterConsole();
	if (tutorials.size() != 0) {
		...
		while (true) {
			try {
				std::cout << '\n';
				cout<<(*it);
				system(std::string("start " + (*it).getLink()).c_str());
				std::cout << "\n[1] Add to watch list.\n";
...
void Console::deleteTutorialWatchList()
{
	...
	bool found = false;
	vector<Tutorial> tutorials = this->service.getWatchlistService();
	for (auto it = tutorials.begin(); it != tutorials.end(); it++)
	...
	if (found)
	{
		...
		this->service.deleteWatchlistService(title, presenter, 1);
	}
	else
...
{
	vector<Tutorial> watch_list = this->service.getWatchlistService();
