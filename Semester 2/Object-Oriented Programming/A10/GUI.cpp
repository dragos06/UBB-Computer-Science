...
void GUI::initGUI()
{
	...
	buttonsLayout->addWidget(redoButton, 1, 1);
	buttonsLayout->addWidget(generateChartButton, 1, 2);
	...
	userLayout->addWidget(watchlistTableView);
	WatchlistTableModel* model = new WatchlistTableModel(service, this);
	watchlistTableView->setModel(model);
	watchlistTableView->resizeColumnsToContents();
...
void GUI::populateWatchlist()
{
	...
	WatchlistTableModel* model = (WatchlistTableModel*)watchlistTableView->model();
	model->layoutChanged();
	watchlistTableView->resizeColumnsToContents();
}
...
void GUI::connectSignalsAndSlots()
{
	...
	QObject::connect(watchlistTableView->selectionModel(), &QItemSelectionModel::selectionChanged, this, &GUI::tableItemChanged);
...
void GUI::generateChartButtonHandler()
{
	...
	*set << service.getNumberTutorialsDuration(1, 3)
		<< service.getNumberTutorialsDuration(3, 5)
		<< service.getNumberTutorialsDuration(5, 10)
		<< service.getNumberTutorialsDuration(10, NULL);
	...
	categories << "1H->3H" << "3H->5H" << "5H->10" << "+10H";
...
{
	try {
		int row = this->watchlistTableView->selectionModel()->currentIndex().row();
		if (row >= 0) {
...
{
	if (this->service.notundo()) {
		QMessageBox::warning(this, "ERROR", "No more undos possible");
		return;
	}
	this->service.undo();
	emit tutorialsUpdatedSignal();
...
{
	if (this->service.notredo()) {
		QMessageBox::warning(this, "ERROR", "No more redos possible");
		return;
	}
	this->service.redo();
	emit tutorialsUpdatedSignal();
...
void GUI::tableItemChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
	if (selected.indexes().isEmpty())
		return;

	int row = selected.indexes().first().row();
	Tutorial t = service.getWatchlistService()[row];
}
