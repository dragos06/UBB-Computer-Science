...
void TutorialRepository::updateTutorial(int index, const Tutorial& t)
{
	if (index < 0 || index >= this->tutorials.size())
		return;
	this->tutorials.erase(this->tutorials.begin() + index);
	this->tutorials.insert(this->tutorials.begin() + index, t);
	this->writeToFile();
}
