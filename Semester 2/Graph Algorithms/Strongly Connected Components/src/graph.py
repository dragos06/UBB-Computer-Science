class Graph:
    ...
    def depth_1st_search(self, vertex_x, visited, processed):
        """
        Depth first search algorithm
        :param vertex_x: The vertex
        :param visited: The set of visited vertices
        :param processed: The list of processed vertices
        :return: The list of processed vertices
        """
        for vertex_y in self.parse_vertices_out(vertex_x):
            if vertex_y not in visited:
                visited.add(vertex_y)
                self.depth_1st_search(vertex_y, visited, processed)
        processed.append(vertex_x)
    ...
    def strongly_connected_components(self):
        ...
        for vertex in self.parse_vertices():
            if vertex not in visited:
                visited.add(vertex)
                self.depth_1st_search(vertex, visited, processed)
        ...
        while len(processed):
            ...
            if vertex not in visited:
                ...
                while len(queue):
                    vertex_x = queue[0]
                    queue.pop(0)
                    for vertex_y in self.parse_vertices_in(vertex_x):
                        if vertex_y not in visited:
                            visited.add(vertex_y)
                            queue.append(vertex_y)
                            components[vertex_y] = c
        return components
