...
class UI:
    ...
    def print_menu():
        ...
        print("[23] Find a Hamiltonian cycle of no more than twice the minimum cost")
        print("[0] Exit\n")
    ...
    def hamiltonian_cycle(self):
        ...
        # shortest_path, cost = self.__graph.find_shortest_path()
        # print(f"Shortest path: {shortest_path} with cost {cost}")


        hamiltonian_cycle, cost = self.__graph.find_hamiltonian_cycle_double_tree()
        if hamiltonian_cycle is None:
            print("No Hamiltonian cycle found")
        else:
            print(f"Hamiltonian cycle: {hamiltonian_cycle} with cost {cost}")

        # hamiltonian_cycle = self.__graph.find_hamiltonian_cycle()
        # print(f"Hamiltonian cycle: {hamiltonian_cycle}")
